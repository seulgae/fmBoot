...
public class TeamController {
    ...
    public String addprofile(HttpServletRequest request, @RequestParam("t_thum") MultipartFile t_thum, String t_no) throws IllegalStateException, IOException {
        ...
        // ?ÑÎ°ú?ùÌä∏ ??webapp ?¥ÎçîÎ•?Ï∞æÏïÑÏ§? webapp ?¥Îçî ?ÜÏùÑ Í≤ΩÏö∞ appdate?àÏùò ?∞Ï∫£ Ï∫êÏãú ?ÑÏãú?Ä???¥Îçî???Ä?•Ïãú??
        // transferTo : ?åÏùº ?∞Ïù¥?∞Î? ÏßÄ?ïÌïú fileÎ°??Ä??
        // getOriginalFilename : ?¥Îùº?¥Ïñ∏?∏Ïùò ?êÎ≥∏ ?åÏùºÎ™?Î∞òÌôò
    ...
    public String insertMember(String str_member,String t_no){
        ...
        return "<script>alert('?±Î°ù???ÑÎ£å?òÏóà?µÎãà??');window.opener.location.reload(); window.close();</script>";
    }
}
